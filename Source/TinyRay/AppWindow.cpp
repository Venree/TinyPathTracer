...
void AppWindow::Render()
{
	...
	if (PATH) 
	{
		pBuffer = m_pPathTracer->GetFramebuffer()->GetBuffer();
		m_pPathTracer->DoPathTrace(m_pScene);
	}
	else
	{
		pBuffer = m_pRayTracer->GetFramebuffer()->GetBuffer();
...
BOOL AppWindow::KeyUp(WPARAM key)
{
	...
	case VK_F7:
		PATH = true;
		m_pPathTracer->m_traceflag = (PathTracer::TraceFlags)(PathTracer::TRACE_PATH);
		break;
	default:
		break;
	}
