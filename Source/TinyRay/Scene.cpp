...
RayHitResult Scene::IntersectByRay(Ray& ray)
{
	...
	for each (Primitive *primitive in m_sceneObjects)
	{
		RayHitResult hit = primitive->IntersectByRay(ray);
		// .t is the position in parametric form
		if (hit.t < result.t && hit.t >= 0)
		{
			result = hit;
		}
	}
